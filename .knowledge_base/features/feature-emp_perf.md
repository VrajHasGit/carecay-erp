...
This module handles the application flow and user interface interactions exclusively for the emp_perf workflow. 
...
1. 
av('emp_perf') in index.html â€” Toggles DOM visibility to display the pg_emp_perf section.
2. enderemp_perf() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderemp_perf() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on emp_perf menu
    API->>Service: nav('emp_perf')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderemp_perf()
