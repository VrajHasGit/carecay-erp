...
This module handles the application flow and user interface interactions exclusively for the pur_dashboard workflow. 
...
1. 
av('pur_dashboard') in index.html â€” Toggles DOM visibility to display the pg_pur_dashboard section.
2. enderpur_dashboard() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderpur_dashboard() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on pur_dashboard menu
    API->>Service: nav('pur_dashboard')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderpur_dashboard()
