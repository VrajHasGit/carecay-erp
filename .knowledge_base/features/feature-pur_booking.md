...
This module handles the application flow and user interface interactions exclusively for the pur_booking workflow. 
...
1. 
av('pur_booking') in index.html â€” Toggles DOM visibility to display the pg_pur_booking section.
2. enderpur_booking() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderpur_booking() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on pur_booking menu
    API->>Service: nav('pur_booking')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderpur_booking()
