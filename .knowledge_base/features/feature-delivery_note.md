...
This module handles the application flow and user interface interactions exclusively for the delivery_note workflow. 
...
1. 
av('delivery_note') in index.html â€” Toggles DOM visibility to display the pg_delivery_note section.
2. enderdelivery_note() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderdelivery_note() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on delivery_note menu
    API->>Service: nav('delivery_note')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderdelivery_note()
