...
This module handles the application flow and user interface interactions exclusively for the pur_inq workflow. 
...
1. 
av('pur_inq') in index.html â€” Toggles DOM visibility to display the pg_pur_inq section.
2. enderpur_inq() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderpur_inq() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on pur_inq menu
    API->>Service: nav('pur_inq')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderpur_inq()
