...
This module handles the application flow and user interface interactions exclusively for the dashboard workflow. 
...
1. 
av('dashboard') in index.html â€” Toggles DOM visibility to display the pg_dashboard section.
2. enderdashboard() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderdashboard() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on dashboard menu
    API->>Service: nav('dashboard')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderdashboard()
