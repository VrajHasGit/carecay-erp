...
This module handles the application flow and user interface interactions exclusively for the test_drive workflow. 
...
1. 
av('test_drive') in index.html â€” Toggles DOM visibility to display the pg_test_drive section.
2. endertest_drive() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via endertest_drive() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on test_drive menu
    API->>Service: nav('test_drive')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via rendertest_drive()
