...
This module handles the application flow and user interface interactions exclusively for the workshop workflow. 
...
1. 
av('workshop') in index.html â€” Toggles DOM visibility to display the pg_workshop section.
2. enderworkshop() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderworkshop() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on workshop menu
    API->>Service: nav('workshop')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderworkshop()
