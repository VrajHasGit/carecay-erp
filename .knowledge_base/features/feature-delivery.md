...
This module handles the application flow and user interface interactions exclusively for the delivery workflow. 
...
1. 
av('delivery') in index.html â€” Toggles DOM visibility to display the pg_delivery section.
2. enderdelivery() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderdelivery() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on delivery menu
    API->>Service: nav('delivery')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderdelivery()
