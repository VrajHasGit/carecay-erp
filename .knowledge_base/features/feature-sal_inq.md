...
This module handles the application flow and user interface interactions exclusively for the sal_inq workflow. 
...
1. 
av('sal_inq') in index.html â€” Toggles DOM visibility to display the pg_sal_inq section.
2. endersal_inq() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via endersal_inq() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on sal_inq menu
    API->>Service: nav('sal_inq')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via rendersal_inq()
