...
This module handles the application flow and user interface interactions exclusively for the stock workflow. 
...
1. 
av('stock') in index.html â€” Toggles DOM visibility to display the pg_stock section.
2. enderstock() in index.html â€” Iterates through the DB collection to render table rows or KPI widgets.
...
- **Reads:** DB collections via enderstock() -> [Link](../data_layer/schema-overview.md)
...
sequenceDiagram
    ...
    participant Service
    participant DB
    Client->>API: Click on stock menu
    API->>Service: nav('stock')
    Service->>DB: DB lookup
    DB-->>Service: Return JSON records
    Service-->>API: DOM Update via renderstock()
